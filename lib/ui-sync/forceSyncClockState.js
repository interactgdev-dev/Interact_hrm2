...
/** Yesterday→today covers overnight shifts without loading full history. */
function recentAttendanceRangeQs() {
  ...
  return `fromDate=${encodeURIComponent(fromDate)}&toDate=${encodeURIComponent(today)}&summary=1`;
}
...
export async function forceSyncClockState(
  ...
  try {
    const res = await fetch(
      `/api/attendance?employeeId=${encodeURIComponent(eid)}&${recentAttendanceRangeQs()}&ts=${Date.now()}`,
      { cache: "no-store" }
    );
    ...
    if (openAttendance?.clock_in) {
      ...
      if (setIntervalId) {
        // Tick from Date.now() after one timezone calc — avoids getParts every second
        const baseElapsed = elapsed;
        const baseNow = Date.now();
        const id = setInterval(() => {
          setTimer(baseElapsed + Math.max(0, Math.floor((Date.now() - baseNow) / 1000)));
        }, 1000);
